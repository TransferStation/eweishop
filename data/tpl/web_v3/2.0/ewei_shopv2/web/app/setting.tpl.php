...
<div class="alert alert-danger">模板消息下发条件说明：当用户在小程序内完成1次微信支付时，可向用户在7天内推送3条模板消息，<b>余额支付则将不能触发</b>
    <br>微信官方调整，小程序模版消息将于2020年1月10日下线，请尽快更换使用小程序订阅消息。</div>
...
<div class="form-group-title">小程序订阅消息</div>
<div class="alert alert-danger">本次订阅消息只能覆盖商家“自营-服饰/鞋/箱包”类目，需要商家前往微信公众号后台-设置-基本设置-服务类目中添加该类目，<b>余额支付则将不能触发</b></div>

<div class="form-group">
    <label class="col-lg control-label">买家支付通知</label>
    <div class="col-sm-9">
        <label class="radio-inline">
            <input class="subscribe-pay subscribe" data-type ='pay' type="radio" value="1" name="set[subscribepay]" <?php  if(!empty($sets['app']['subscribepay'])) { ?>checked<?php  } ?>> 启用</label>
        <label class="radio-inline">
            <input class="subscribe-pay subscribe" data-type ='pay'  type="radio" value="0" name="set[subscribepay]" <?php  if(empty($sets['app']['subscribepay'])) { ?>checked<?php  } ?>> 不启用</label>
    </div>
</div>

<div class="form-group">
    <label class="col-lg control-label">卖家发货通知</label>
    <div class="col-sm-9">
        <label class="radio-inline">
            <input class="subscribe-send subscribe" data-type ='send'   type="radio" value="1" name="set[subscribesend]" <?php  if(!empty($sets['app']['subscribesend'])) { ?>checked<?php  } ?>> 启用</label>
        <label class="radio-inline">
            <input class="subscribe-send subscribe" data-type ='send'   type="radio" value="0" name="set[subscribesend]" <?php  if(empty($sets['app']['subscribesend'])) { ?>checked<?php  } ?>> 不启用</label>
    </div>
</div>

<div class="form-group">
    <label class="col-lg control-label">自动发货通知</label>
    <div class="col-sm-9">
        <label class="radio-inline">
            <input class="subscribe-autosend subscribe" data-type ='autosend'   type="radio" value="1" name="set[subscribeautosend]" <?php  if(!empty($sets['app']['subscribeautosend'])) { ?>checked<?php  } ?>> 启用</label>
        <label class="radio-inline">
            <input class="subscribe-autosend subscribe" data-type ='autosend' type="radio" value="0" name="set[subscribeautosend]" <?php  if(empty($sets['app']['subscribeautosend'])) { ?>checked<?php  } ?>> 不启用</label>
    </div>
</div>

<div class="form-group">
    <label class="col-lg control-label">买家收货通知</label>
    <div class="col-sm-9">
        <label class="radio-inline">
            <input class="subscribe-receive subscribe" data-type ='receive' type="radio" value="1" name="set[subscribereceive]" <?php  if(!empty($sets['app']['subscribereceive'])) { ?>checked<?php  } ?>> 启用</label>
        <label class="radio-inline">
            <input class="subscribe-receive subscribe" data-type ='receive'  type="radio" value="0" name="set[subscribereceive]" <?php  if(empty($sets['app']['subscribereceive'])) { ?>checked<?php  } ?>> 不启用</label>
    </div>
</div>
...
//        });
    ...
    function changeStatus(item) {
        var check = item.val();
        if (check == 1) {
            setTemplate(item.data('type'));
        }
        return;
        FoxUI.loader.hide('mini')
    ...
    function setTemplate(type) {
        tip.confirm('您确认开启吗?', function (res) {
            $.ajax({
                url: "<?php  echo webUrl('app/setting/setTemplate')?>",
                dataType: 'json',
                data: {
                    type: type
                },
